                    ...
                    <tbody>
                        ...
                        @foreach ($menus as $index => $item)
                            <tr class="border-bottom">
                                ...
                                <td><a href={{ route('EditMenu', ['id' => $item->id]) }}
                                        class="border border-0 button-revise">修改</a></td>
                            </tr>
                        @endforeach
                        ...
                        @foreach ($icons as $index => $item)
                            <tr class="border-bottom">
                                ...
                                <td class="height-100">
                                    ...
                                    <textarea style="height: 75px" name="icon_url{{ $item->id }}" id="" class="link">{{ $item->icon_url }}</textarea>
                                </td>
                                <td class="upload-td height-100">
                                    <label style="position: relative;
                            top: 20px;" for="upload{{ $item->id }}"
                                        class="border upload-label">選擇檔案</label>

                                    <input style="position: relative;
                            top: 0px;" sty
                                        type="file" name="upload{{ $item->id }}" id="upload{{ $item->id }}"
                                        class="upload-input isup" accept="image/png, image/jpeg,image/svg, image/gif">
                                </td>
                                <td class="height-100">
                                ...
                                <td class="height-100">
                                    ...
                                    <button data-key="{{ route('icon.update', $item->id) }}" type="button" disabled
                                        class="border button-store">儲存
                                    </button>
                                </td>
